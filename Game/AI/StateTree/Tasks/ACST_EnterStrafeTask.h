...
#include "ACST_EnterStrafeTask.generated.h"
...
struct FEnterStrafeTaskInstanceData
{
    GENERATED_BODY()
...
struct DAZADA_API FACST_EnterStrafeTask : public FStateTreeTaskCommonBase
{
    GENERATED_BODY()

    using FInstanceDataType = FEnterStrafeTaskInstanceData;

    virtual const UStruct* GetInstanceDataType() const override
    {
        return FEnterStrafeTaskInstanceData::StaticStruct();
    }
        ...
        const FStateTreeTransitionResult& Transition) const override
    {
        ...
        FEnterStrafeTaskInstanceData& Data = Context.GetInstanceData(*this);

        // Safety checks
        if (!Data.Actor)
        {
            UE_LOG(LogTemp, Warning, TEXT("FEnterStrafeTaskInstanceData: Actor is null."));
            return EStateTreeRunStatus::Failed;
        }
        Data.Actor->lockCamera = true;
        Data.Actor->bUseControllerRotationYaw = true;
